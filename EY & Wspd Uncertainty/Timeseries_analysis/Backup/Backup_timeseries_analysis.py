...
def main():  # Function to create output folder where all the files are saved. This is where I first create location folder, then the graph folder and also create a notepad for saving all variables.
    ...
    output_dir = r"C:\Kshitij stuff\Horizontal Uncertainty Check\Output_newnew"
...
def plot_scatter_measured_vs_predicted(
    x, y,
    xlabel, ylabel,
    title, filename,
    extra_info_text="",
    point_color='blue', fit_color='red',
    alpha=0.6, figsize=(8, 6)
):
    """
    General scatter plot with regression and stats between predicted vs. measured windspeed.

    Parameters:
        x (pd.Series): Predicted values
        y (pd.Series): Measured values
        xlabel (str): X-axis label
        ylabel (str): Y-axis label
        title (str): Plot title
        filename (str): Output filename
        extra_info_text (str): Optional text block for annotations
        point_color (str): Color of scatter points
        fit_color (str): Color of regression line
        alpha (float): Transparency of points
        figsize (tuple): Figure size

    Returns:
        While Measured vs self prediction doesnt do much for me, the measured vs. cross predicted is interesting to look at bc it gives a good idea about whats going on with my wind data.
    """
    ...
    ax.scatter(x, y, s=15, color=point_color, alpha=alpha, label='Data')
    ax.plot(x, line, color=fit_color, label='Fit line')
    max_val = max(max(x), max(y))
    ax.plot([0, max_val], [0, max_val], 'k--', linewidth=1, label='1:1 line')

    # Annotations
    stats_text = f"STDDEV: {stddev:.2f} m/s\nBIAS: {bias:.2f} m/s\nMAE: {mae:.2f} m/s"
    ax.text(0.05, 0.95, stats_text, transform=ax.transAxes, fontsize=8, verticalalignment='top')
    ...
    ax.text(0.65, 0.25, fit_text, transform=ax.transAxes, fontsize=8)

    # Labels and display
    ax.set_xlabel(xlabel, fontsize=8)
    ax.set_ylabel(ylabel, fontsize=8)
    ax.set_title(title, fontsize=8, color='teal')
    ax.grid(True)
    ax.legend()
    fig.tight_layout()
    plt.close(fig)

    return fig, filename, slope, intercept, r2, bias, mae, pearson_corr_coeff, stddev
...
#####################################
    ...
    # Scatter plot b/w measured and Self by calling function:
    ...
    fig_self, fname_self, slope_self, intercept_self, r2_self, bias_self, mae_self, pearson_corr_coeff_self, stddev_self = plot_scatter_measured_vs_predicted(
        x=df_combined_filtered['Self_windspeed'],
        y=df_combined_filtered['measured_windspeed'],
        xlabel=f'Self-predicted windspeed ({Device_self}) [m/s]',
        ylabel=f'Measured windspeed ({Device_measured}) [m/s]',
        title='Scatter plot Measured vs. Self windspeed Comparison',
        filename='Scatter plot Measured vs. Self windspeed Comparison.png'
    )
    figures.append((fig_self, fname_self))

    # Scatter plot b/w measured and predicted by calling my function:

    x = df_combined_filtered['predicted_windspeed']
    y = df_combined_filtered['measured_windspeed']

    extra_info = f"""Measurement with {Device_measured} at Z-height: {Z_height_measured} m
    ...
    fig_cross, fname_cross, slope_cross, intercept_cross, r2_cross, bias_cross, mae_cross, pearson_corr_coeff_cross, stddev_cross = plot_scatter_measured_vs_predicted(
        x=df_combined_filtered['predicted_windspeed'],
        y=df_combined_filtered['measured_windspeed'],
        xlabel=f'Cross Predicted windspeed ({Device_measured}) [m/s]',
        ylabel=f'Measured windspeed ({Device_predicted}) [m/s]',
        title='Scatter plot: Measured vs. Predicted Comparison',
        filename='Scatter plot Measured vs. Predicted windspeed Comparison.png',
        extra_info_text=extra_info,
        figsize=(10, 6)
    )
    figures.append((fig_cross, fname_cross))
    ...
    distance_percent = (dist_measured_predicted/distance_B)
...
output_path = rf"C:\Kshitij stuff\Horizontal Uncertainty Check\Output_newnew\Ω_Collected output\Collected output_{today}.xlsx"
...
for xi, yi, label in zip(x, y, df_graph_positive['Location']):
    plt.text(xi, yi+0.01, str(label), fontsize=8, ha='center', va='bottom')
plt.xlabel('T-RIX(%)')
plt.ylabel('Energy yield deviation(Fractional)')
plt.title('T_RIX vs Energy yield deviation')
plt.grid(True)
plt.tight_layout()
plt.show()

# Plot for 0.7*T_RIX + 0.3*Distance/Distance(B) vs Energy yield deviation
x_RIX70 = 0.7* df_graph_positive['TRIX'] + 0.3*(df_graph_positive['distance %']*100)  # Converting to percentage for better scale
y_RIX70 = df_graph_positive['Energy yield deviation']
plt.figure(figsize=(8, 6))
plt.scatter(x_RIX70, y_RIX70, color='orange', alpha=0.7)
for xi, yi, label in zip(x_RIX70, y_RIX70, df_graph_positive['Location']):
    plt.text(xi, yi+0.01, str(label), fontsize=8, ha='center', va='bottom')
plt.xlabel('0.7*T-RIX + 0.3*Distance/Distance(B) (%)')
plt.ylabel('Energy yield deviation(Fractional)')
