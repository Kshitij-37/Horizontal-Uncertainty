"""
Wind Speed Horizontal Extrapolation Uncertainty Calculator (TR_v4.0)

POST-THESIS model variant. Identical to TR_v3.0 EXCEPT the roughness feature is
distance-scaled (proximity gate): the roughness contribution is damped for close
pairs before saturation. Bundles the matching post-thesis fit
(Post-thesis corrections/Results, 40 pairs), whose parameters were fit WITH the gate.
Use TR_v3.0 for the thesis-final (un-gated) model.
...
5-feature pair-level Bayesian model (Student-t, nu~14):
    log(sigma) = log_sigma0
               ...
               + gamma_roughness * z_roughness_sat      (roughness distance-scaled)
               + gamma_dz        * z_dz_sat

    mu = beta_dz * z_dz  (signed height-dependent bias)

    roughness proximity gate:  wm_abs_roughness *= 1 - exp(-distance_m / 500)
...
if not os.path.isfile(MODEL_JSON):
    # Post-thesis (gated) fit lives at repo root under "Post-thesis corrections/Results".
    MODEL_JSON = os.path.normpath(os.path.join(
        os.path.dirname(__file__), os.pardir, os.pardir,
        "Post-thesis corrections", "Results",
...
def compute_features(wtg_info, mm_data, wtg_data, freq_norm):
    ...
    proximity_gate = 1.0 - np.exp(-distance_m / PROXIMITY_GATE_SCALE)
    wm_abs_roughness = wm_abs_roughness * proximity_gate
...
class CalculatorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("WS Horizontal Extrapolation Uncertainty Calculator — Multi-Reference (v4.0, roughness distance-scaled)")
    ...
    def _build_ui(self):
        ...
        ttk.Label(bottom_bar, text="TR_v4.0 (Multi-Reference, roughness distance-scaled)",
