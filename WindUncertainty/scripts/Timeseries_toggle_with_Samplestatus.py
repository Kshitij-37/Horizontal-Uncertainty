...
excel_meteo = os.path.join(_cfg["input_data_dir"], "Device data.xlsx")
...
GRAPHS = {
    ...
    'device_diagram': False,   # WIP, still looks trash
    'dashboard': True,
    'trix_plots': True,
}
...
if GENERATE_GRAPHS and GRAPHS.get('device_diagram'):
    _img_dir = _cfg["images_dir"]
    image_cache = {
        "img_lidar_measured":mpimg.imread(os.path.join(_img_dir, "lidar measured.png")),
        "img_lidar_predicted":mpimg.imread(os.path.join(_img_dir, "lidar predicted.png")),
        "img_mm_measured":mpimg.imread(os.path.join(_img_dir, "metmast measured.png")),
        "img_mm_predicted":mpimg.imread(os.path.join(_img_dir, "metmast predicted.png"))
    }
else:
    image_cache = {}
...
#####################################
    ...
    # Keep only rows where SampleStatus == 0 (valid timestamps)
    sample_status_cols = [col for col in df_measured.columns if col.lower().startswith('samplestatus')]
    if sample_status_cols:
        sample_status_col = sample_status_cols[0]
        df_measured = df_measured[pd.to_numeric(df_measured[sample_status_col], errors='coerce') == 0]
    ...
    expected_counts = monthly_counts.index.to_series().apply(expected_intervals_per_month).to_numpy()
    ...
    if GENERATE_GRAPHS and GRAPHS.get('monthly_quality'):
    ...
    if GENERATE_GRAPHS and GRAPHS.get('mae_analysis'):
    ...
    # ── Always compute stats (independent of graph generation) ──
    x_self = df_combined_filtered['Self_windspeed'].values
    y_meas = df_combined_filtered['measured_windspeed'].values
    x_cross = df_combined_filtered['predicted_windspeed'].values

    # Self stats
    slope_self_lr, intercept_self_lr, _, _, _ = linregress(x_self, y_meas)
    r2_self = r2_score(y_meas, slope_self_lr * x_self + intercept_self_lr)
    bias_self = (y_meas - x_self).mean()
    mae_self = np.abs(y_meas - x_self).mean()

    # Cross stats
    slope_cross, intercept_cross, _, _, _ = linregress(x_cross, y_meas)
    r2_cross = r2_score(y_meas, slope_cross * x_cross + intercept_cross)
    bias_cross = (y_meas - x_cross).mean()
    mae_cross = np.abs(y_meas - x_cross).mean()
    pearson_corr_coeff_cross, _ = pearsonr(x_cross, y_meas)
    stddev_cross = (y_meas - x_cross).std()


    # Scatter: Measured vs Self
    if GENERATE_GRAPHS and GRAPHS.get('scatter_self'):
    ...
    if GENERATE_GRAPHS and GRAPHS.get('scatter_predicted'):
        fig, meta = mg.scatter_validation(
    ...
    if GENERATE_GRAPHS and GRAPHS.get('dashboard'):
...
if WRITE_COLLECTED_EXCEL:
    # Collecting all data in 1 file:
    search_dir = _cfg["output_dir"]
    ...
    output_path = os.path.join(_cfg["output_dir"], f"Collected output_{today}.xlsx")
    ...
    if GENERATE_GRAPHS and GRAPHS.get('trix_plots'):
