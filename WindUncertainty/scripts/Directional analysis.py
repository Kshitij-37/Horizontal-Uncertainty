...
# Speed up factors
speed_up_factors = os.path.join(_cfg["input_data_dir"], "Speed up factors.xlsx")
...
fallback_folder = _cfg.get("fallback_folder", "")
...
for instance_id, info in project_txt_paths.items():
    try:
        ...
        for txt_predicted in cross_files:
            ...
            # --- Filter non-physical wind speeds (negative values = sensor errors) ---
            ws_cols = ['windspeed_measured', 'windspeed_self', 'windspeed_predicted', 'windspeed_pred_true']
            for ws_col in ws_cols:
                if ws_col in df_combined.columns:
                    n_neg = (df_combined[ws_col] < 0).sum()
                    if n_neg > 0:
                        print(f"  ⚠️ {instance_id}: {n_neg} negative values in {ws_col} → set to NaN")
                        df_combined.loc[df_combined[ws_col] < 0, ws_col] = np.nan
            ...
            df_combined['dir_bin_pred_true'] = df_combined['winddirection_pred_true'].apply(direction_to_compass)

            grouped_self = df_combined.groupby('dir_bin_self')['energy_self'].sum().reindex(compass_labels,fill_value=0)
            grouped_pred = df_combined.groupby('dir_bin_predicted')['energy_predicted'].sum().reindex(compass_labels,fill_value=0)
            ...
            energy_summary = pd.DataFrame({
                ...
                f'Energy_Yield_{self_project}': grouped_self.values, # now is kWh
                f'Energy_Yield_{predicted_project}': grouped_pred.values, # now is kWh
                ...
                'Mean_windspeed_predicted_true': (
                    df_combined.groupby("dir_bin_pred_true")['windspeed_pred_true'].mean().reindex(compass_labels).values
                )
            }).set_index("Direction")
            ...
            out_path = os.path.join(_cfg["directional_analysis_dir"], f"Directional_analysis_{measured_project}_{predicted_project}.xlsx")
